...
export function HeroStatus({ robot, status, loading }: Props) {
  ...
  return (
    <div className={`bg-gradient-to-r ${cfg.color} rounded-2xl border ${cfg.border} p-6`}>
      ...
      {/* Offline warning — takes precedence over localization. */}
      {status && status.online === false && (
        <div className="mt-3 flex items-center gap-2 text-red-400 text-sm bg-red-900/30 rounded-lg px-3 py-2">
          ...
          Robot offline &mdash; commands will fail until it reconnects
        </div>
      )}

      {/* Localization warning (Gausium only in practice — Pudu polling does
          not expose localization state, so the server sets localized: true
          for Pudu). */}
